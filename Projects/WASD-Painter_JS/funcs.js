...
function mouseOnCanvas() {
    return compr(mouseX, width) && compr(mouseY, height);
}
...
function distance(arg1, arg2 = null, arg3 = null, arg4 = null) {
    if (typeof (arg1) == "number" && typeof (arg2) == "number") {
        x1 = arg1;
        y1 = arg2;
        if (typeof (arg3) == "number" && typeof (arg4) == "number") {
    ...
    return Math.abs(((x1 - x2) ** 2 + (y1 - y2) ** 2) ** (0.5));
}
...
function remap(value, min1, max1, min2, max2) {
    ...
    if (max2 >= min2) {
        final = ((value - min1) / (max1 - min1) * (max2 - min2)) + min2;
    } else {
        final = min2 - ((value - min1) / (max1 - min1) * (min2 - max2));
    }
    return final;
}